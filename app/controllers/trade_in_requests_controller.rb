...
class TradeInRequestsController < StoreController
  def new
    set_form_variables
  ...
  def set_form_variables
    ...
    @trade_in_models = TradeInModel.all.includes(:trade_in_category).order(:order)
    @accessories_taxon_id = Spree::Taxon.find_by(name: 'Accesorios').id
    @taxons = Spree::Taxon.where.not(id: @accessories_taxon_id).where(depth: 1)
