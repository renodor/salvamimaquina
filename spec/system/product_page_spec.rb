...
RSpec.describe 'Product page', type: :system, js: true do
  ...
  let!(:product) { create(:smm_product, name: 'Cool product!', description: 'This is a cool product.') }
  ...
  context 'when product has variants' do
    ...
    let!(:image) { create(:smm_image, viewable: variant, attachment: Rails.root.join('spec', 'fixtures', 'product_images', 'iphone-15.png').open) }
    ...
    let!(:image3) { create(:smm_image, viewable: variant3, attachment: Rails.root.join('spec', 'fixtures', 'product_images', 'iphone-14a.jpg').open) }
    let(:stock_item) { create(:stock_item, variant: variant) }
    let(:stock_item2) { create(:stock_item, variant: variant2) }
    let(:stock_item3) { create(:stock_item, variant: variant3) }

    before do
      stock_item.set_count_on_hand(1)
      stock_item2.set_count_on_hand(0)
      stock_item3.set_count_on_hand(3)
    ...
    it 'displays product main photo and thumbnails' do
      ...
      expect(find('#product-images #thumbnails .thumbnail.selected img')['src'])
        .to eq("https://cloudinary.com/salvamimaquina/image/upload/c_fit,d_test:products:product-image-placeholder.jpg,h_48/v1/test/products/#{image3.attachment.key}")
    end
    ...
    it 'allows to change quantity' do
